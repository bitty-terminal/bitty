...
- **Six-shell coverage evidence (CTX-0588, M1-27, Issue #1153):**
  ...
  (bash/zsh/fish/PowerShell/cmd/nushell) and asserts the shell's own output
  never fabricates `OSC 7`/`OSC 133` state. Each available shell then emits
  injected `OSC 7` (cwd) and `OSC 133` A/B/C/D (`D;0`) sequences through the
  shell itself, and the suite asserts the parser/state records exactly the
  one cwd report and the ordered four-zone cycle with exit code 0. A
  prompt-text purity test proves no prompt heuristic can synthesize cwd or
