...
fn final_headless_integration_end_to_end() {
    ...
    // CTX-0359: the primary grid stays with its owner leaf (the focused
    // leaf when the primary attached — v:1 here), so a faithful replay must
    // mirror focus too (rt focuses v2 above): focus is frame state, and
    // session-less non-owner tiles stay erased instead of duplicating
    // primary.
