...
fn search_ui_refresh_preserves_current_clamped_and_auto_refresh_headless() {
    ...
    // CTX-0375: the content grid excludes the 13px per-side decoration inset,
    // so 772x624 maps to an 81x30 content grid (window 84x32 cells) under the
    // default 9x19 cells and 8px padding: same 81 columns, 24 -> 30 rows.
    rt.handle_resize(PhysicalSize::new(772, 624))
        .expect("resize must succeed");
    assert_eq!(rt.state().width(), 81);
