...
- Ownership: bitty **CTX-0087** — _Manual smoke checklist vs mainstream terminals_ (CTX-0099 extends).
  ...
  - CTX-0099: Priority: P0 | Area: qa/compat | Labels: feat,area:qa,P0 | Milestone: v0.1.0 | RFC: OQ-001 | Task: CTX-0099 — Branch `carryctx/ctx-0099` — Base `a8735d0` (CTX-0098) — Agent `core-implementer-0099` — Worktree `.worktrees/ctx-0099` — Revision `a8735d0+` — Evidence `recording/references/bitty/*.snapshot.json` (39 dumps) + `tests/compat/*/corpus/*dogfooding*.bin` (9 new) + `cargo test -p bitty-compat-lab --test dogfooding_corpus` PASS.
- Companion tasks:
  - **CTX-0085** — differential comparator (`tests/compat` grid hash/damage vs reference dumps, headless `forbid(unsafe)`).
  - **CTX-0086** — pinned reference dumps (`ghostty`/`kitty`/`wezterm`/`alacritty`/`xterm` under `recording/references/<emulator>/`, revision+license).
  - **CTX-0099** — daily-driver dogfooding corpus (zsh/bash/fish, nvim/tmux/fzf/htop/ssh, alt-screen/mouse/resize/OSC 7/8/133/clipboard/Kitty/IME/DPI, headless bounded, differential vs Ghostty/Kitty/WezTerm/Alacritty via `recording/references/bitty`).
- Scope: human-in-loop manual regression — the same 7 surfaces as the automated compat lab (`prompt marks`, `nvim`, `tmux`, `fzf/htop`, `ssh`, `mouse`, `kitty keyboard/modifyOtherKeys`) plus shell-integration parity (`ghostty`/`kitty` vs `bitty`). Automated leg stays bounded/headless; this doc is the **manual extension** — CTX-0099 adds headless dogfooding corpus and `recording/` traces, windowed `grim` remains manual opt-in.
...
| 4.4 | Headless ssh corpus | `cat tests/compat/shell/corpus/ssh-*.bin` equivalent `corpus_ssh()` replay       | bounded ≤8 KiB, `TitleChanged` observed in `cold_queue_len ≤256`                                         | bounded ≤8 KiB PASS, `TitleChanged` observed `recording/references/bitty/tui-03*.json` `cold_queue ≤256` | `cargo test --test dogfooding` ssh row + `cargo test -p bitty-compat-lab --test harness` PASS | PASS |
...
- Reference dumps (CTX-0086, read-only): `recording/references/<emulator>/` per `recording/references/README.md` (ghostty `8867c37` MIT, kitty `087b8c3` GPL-3.0, wezterm `f93d903` MIT, alacritty/xterm pinned separately).
...
- Run header: `date: 2026-08-31  host: cachyos-hyprland  bitty: a8735d0+  ghostty: 8867c37 (recording/references/ghostty)  kitty: 087b8c3  wezterm: f93d903  alacritty: ede2ac1  xterm: 9489b20  nvim: headless corpus  tmux: headless corpus  fzf: headless corpus  htop: headless corpus` (reference revisions from `recording/references/README.md`).
...
- Snapshots: `cargo run -p bitty-compat-lab --bin collect_dumps --locked` wrote 39 snapshots (30 baseline + 9 dogfooding) to `recording/references/bitty/` and `tmp/references/bitty/` mirrored to `recording/references/bitty/` (umbrella) — each `<16 KiB`, `80×24`, `CANONICAL_HASH_VERSION 1`, `state_hash` deterministic.
