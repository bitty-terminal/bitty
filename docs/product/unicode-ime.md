...
- Wire `tests/compat/unicode/corpus/*.bin` (and existing categories) into a `cargo test` harness that calls `harness::parse_bounded` + `State` and optionally diffs against checked-in `unicode/reference/*.txt` grid dumps (Phase C follow-up after `vttest` pin in `recording/references/`).
...
- Land `terminfo/bitty.ti` draft and record the `terminfo` revision in `recording/references/` when accepted.
